...
export class Draw{
    ...
    public squareOnGrid(position: GridCoordinate, size: number, color: string = "black", borderColor?: string, borderWidth: number = 0, offset?: ScreenCoordinate){
        ...
        let pos: ScreenCoordinate = position.toScreenCoordinate(size);
        if(offset !== undefined){
            pos = pos.add(offset);
        } 
